...
const PREVIOUS_MINIMAL_LOCATION_PROMPT = [
...
function getMessageIndexFromEventArgs(args) {
    const context = getSillyTavernContext();
    for (const value of args) {
        const candidate = typeof value === 'number'
            ? value
            : value && typeof value === 'object'
                ? value.id ?? value.message_id ?? value.messageId ?? value.mesId ?? value.mes_id
                : null;
        const index = Number(candidate);
        if (Number.isInteger(index) && index >= 0 && context?.chat?.[index]) return index;
    }
...
function getFallbackLocationMatch() {
    const currentMatch = findMappingByLocationName(getCurrentLocationName());
    if (currentMatch) return currentMatch;
    const startUid = String(getSettings().startLocations[activeWorldName] ?? '');
    const startMapping = getCurrentWorldMappings()[startUid];
    const startEntry = getEntryByUid(startUid);
    return startMapping && startEntry ? { uid: startUid, mapping: startMapping, entry: startEntry } : null;
}

async function correctInvalidReportedLocation(args, text, invalidLocation) {
    const context = getSillyTavernContext();
    const messageIndex = getMessageIndexFromEventArgs(args);
    const message = context?.chat?.[messageIndex];
    if (!message || message.is_user) return false;
    const fallback = getFallbackLocationMatch();
    if (!fallback) return false;
    const fallbackName = getEntryLabel(fallback.entry);
    const replacementLine = getSettings().locationLineFormat === LOCATION_LINE_FORMATS.hidden
        ? `<!-- Location: ${fallbackName} -->`
        : `Location: ${fallbackName}`;
    const lines = String(text ?? '').split(/\r?\n/);
    let replaced = false;
    for (let index = lines.length - 1; index >= Math.max(0, lines.length - 8); index--) {
        if (/\[LBM_LOCATION\s*:/i.test(lines[index])) {
            lines[index] = lines[index].replace(/\[LBM_LOCATION\s*:\s*[^\]\r\n]+\]/i, `[LBM_LOCATION: ${fallbackName}]`);
            replaced = true;
            break;
        }
        if (/^(?:\s*<!--\s*)?(?:\[LBM_LOCATION\s*:|(?:current\s+)?location\s*[:=-])/i.test(lines[index])) {
    ...
    setStatus(`Rejected unknown location "${invalidLocation}"; kept "${fallbackName}".`, true);
    return true;
}
...
async function processLocationMarkerText(text) {
    ...
    const locationName = getReportedLocationName(text);
    if (isIgnoredLocationMarker(locationName)) {
        return false;
    }
...
async function onChatMessageForLocationMarker(...args) {
    try {
        ...
        const text = extractMessageTextFromEventArgs(args);
        const reportedLocation = getReportedLocationName(text);
        let processed = await processLocationMarkerText(text);
        if (!processed && reportedLocation && !findMappingByLocationName(reportedLocation)) {
            processed = await correctInvalidReportedLocation(args, text, reportedLocation);
        }
        if (processed) {
